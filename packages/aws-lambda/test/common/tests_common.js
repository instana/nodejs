...
exports.registerTests = function registerTests(handlerDefinitionPath) {
  ...
  describe('API Gateway/EUM - Server-Timing array header already present', function() {
    ...
    it('must add our Server-Timing value to the header array', () => {
      let serverTimingValue = null;
      return verify(control, false, true, true, 'aws:api.gateway')
        .then(() => {
          const response = control.getLambdaResults()[0];
          const serverTimingValueArray = getHeaderCaseInsensitive(response.headers, 'server-timing');
          expect(serverTimingValueArray).to.be.an('array');
          serverTimingValue = serverTimingValueArray.find(val => /^intid;desc=[0-9a-f]+/.test(val));
          expect(serverTimingValue).to.be.a('string');
        })
        .then(() => control.getSpans())
        .then(spans =>
          expectOneMatching(spans, span => {
