...
module.exports = function () {
  mochaSuiteFn('OTLP format', function () {
    ...
    it('should send spans in OTLP format when enabled via code configuration', () =>
      controls.sendRequest({ method: 'GET', path: '/' }).then(async () => {
    ...
    it('should send spans in Instana format when OTLP is disabled', async () => {
      ...
      try {
        await disabled.sendRequest({ method: 'GET', path: '/' });
        ...
        expect(httpSpan.data.http.url).to.equal('/');
    ...
    it('should send spans in OTLP format when enabled via environment variable', async () => {
      ...
      try {
        await env.sendRequest({ method: 'GET', path: '/' });
    ...
    it.skip('should send spans in OTLP format when enabled by agent configuration', async () => {
      ...
      try {
        await cfg.sendRequest({ method: 'GET', path: '/' });
