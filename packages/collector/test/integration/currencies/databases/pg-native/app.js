...
client.connect(
  `host=${process.env.INSTANA_CONNECT_POSTGRES_HOST || 'localhost'} port=5432 dbname=${
    process.env.INSTANA_CONNECT_POSTGRES_DB
  } user=${process.env.INSTANA_CONNECT_POSTGRES_USER} password=${process.env.INSTANA_CONNECT_POSTGRES_PASSWORD}`,
  err => {
    if (err) {
