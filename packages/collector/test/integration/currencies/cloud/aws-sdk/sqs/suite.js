    ...
    describe('tracing enabled, no suppression', function () {
      ...
      function verifySpans(receiverControls, _senderControls, spans, apiPath, messageId, withError, isBatch) {
        ...
        if (withError !== 'publisher') {
          ...
          // For non-batch scenarios the span count is deterministic:
          //  1 HTTP entry + 1 SQS exit + 1 SQS entry + 1 HTTP exit = 4.
          // For batch, SQS may deliver messages across multiple receive calls producing additional
      ...
      function verifySQSEntry(receiverControls, spans, parent, messageId, withError, isBatch) {
         ...
         */
        const operation = isBatch ? expectAtLeastOneMatching : expectExactlyOneMatching;
