...
['latest', 'v4'].forEach(version => {
  mochaSuiteFn(`tracing/express@${version} http with proxy`, function () {
    ...
    before(async () => {
      ...
      await expressProxyControls.start({ useGlobalAgent: true, expressControls, EXPRESS_VERSION: version });
    });
