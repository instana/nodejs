...
const loggers = process.env.LOGGING_DISABLED && JSON.parse(process.env.LOGGING_DISABLED);
...
app.put('/com.instana.plugin.nodejs.discovery', (req, res) => {
  ...
  if (kafkaTraceCorrelation != null || extraHeaders.length > 0 || enableSpanBatching || ignoreEndpoints || loggers) {
    response.tracing = {};
    ...
    if (loggers) {
      response.tracing.logging = loggers;
    }
  }
  res.send(response);
});
