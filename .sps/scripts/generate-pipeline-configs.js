...
function generateOne(t) {
  if (t === 'default') {
    ...
    const script = [
      ...
      '      "https://api.github.com/repos/$REPO/commits/$GIT_COMMIT/statuses?per_page=100&page=$page")',
      '    local count',
      ...
      '    page=$((page + 1))',
      '  done',
      '  # Merge all pages into one flat JSON array',
      '  echo "$all" | python3 -c "',
      'import sys',
      'data = sys.stdin.read()',
      '# Each page is a separate JSON array — parse and merge',
      'import re, json',
      'arrays = re.findall(r\'\\[.*?\\]\', data, re.DOTALL)',
      'merged = []',
      'for a in arrays:',
      '    try: merged.extend(json.loads(a))',
      '    except: pass',
      ...
      '  OTHERS=$(echo "$STATUSES" | python3 -c "import sys,json; d=json.load(sys.stdin); print(len(set(r[\'context\'] for r in d if not r[\'context\'].startswith(\'tekton/pr-code-checks-verify\'))))")',
      ...
      'for r in d:',
      '    c=r[\'context\']',
      '    if c not in seen and c.endswith(\'/code-unit-tests\') and not c.startswith(\'tekton/pr-code-checks-verify\') and r[\'state\']==\'pending\':',
      '        pending+=1; seen.add(c)',
      'print(pending)")',
