...
function generateOne(t) {
  if (t === 'default') {
    ...
    const script = [
      ...
      '',
      '# Helper: download the COS result file for a single completed check-run name.',
      '# Skips silently if already downloaded or if COS has no file for it.',
      'download_cos_result() {',
      '  local check_name="$1"',
      '  # Derive the task slug: strip leading "pr-code-checks-" if present',
      '  local slug="${check_name#pr-code-checks-}"',
      ...
      '  OTHERS=$(echo "$STATUSES" | python3 -c "import sys,json; d=json.load(sys.stdin); print(sum(1 for r in d.get(\'check_runs\',[]) if r[\'name\']!=\'pr-verify\'))")',
      ...
      '    -H "Accept: application/vnd.github+json" \\',
      '    "https://api.github.com/repos/$REPO/commits/$GIT_COMMIT/check-runs?per_page=100")',
      '  # Download COS file for every check that just became completed',
      '  while IFS= read -r check_name; do',
      '    download_cos_result "$check_name"',
      "  done < <(echo \"$STATUSES\" | python3 -c 'import sys,json; d=json.load(sys.stdin); [print(r[\"name\"]) for r in d.get(\"check_runs\",[]) if r[\"status\"]==\"completed\" and r[\"name\"]!=\"pr-verify\"]')",
      ...
      'ALL_TESTS=$(find "$REPO_PATH/packages/collector/test/integration" \\',
      '  -name "*.test.js" \\',
