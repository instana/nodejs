...
function uploadTestFilesLines(taskSlug) {
  return [
    ...
    'GIT_COMMIT="$(cd "$WORKSPACE/$(load_repo app-repo path)" && git rev-parse HEAD^2 2>/dev/null || git rev-parse HEAD)"',
...
function generateOne(t) {
  if (t === 'default') {
    ...
    const script = [
      ...
      'COS_API_KEY="$(get_secret ibm-object-storage-api-key)"',
      '# SPS merges the PR into base — HEAD^2 is the PR branch tip (the commit GitHub tracks checks on)',
      'GIT_COMMIT="$(cd "$WORKSPACE/$(load_repo app-repo path)" && git rev-parse HEAD^2 2>/dev/null || git rev-parse HEAD)"',
      'REPO="instana/nodejs"',
      '',
