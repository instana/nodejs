...
After cloning the repository, run `npm install` in the root of the repository. This will install `lerna` as a local dependency and also bootstrap all packages (by running `npm install` in the individual packages, `packages/core`, `packages/collector`, ...). It is necessary to install `lerna` v7 globally in order to perform lerna commands straight from the command line `npm install -g lerna@7`.
