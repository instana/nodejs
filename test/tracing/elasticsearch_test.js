...
describe('tracing/elasticsearch', function() {
  ...
  it('must report errors caused by missing indices', function() {
    return expressElasticsearchControls.get({
    ...
    .then(function() {
      return utils.retry(function() {
        ...
        .then(function(spans) {
          expect(spans).to.have.lengthOf(2, 'Should not generate an exit span when there is already one. Spans: ' +
          ...
          utils.expectOneMatching(spans, function(span) {
            ...
            expect(span.data.elasticsearch.error).to.match(/no such index|missing/ig);
          });
        });
      });
    });
    ...
    .then(function(documentId) {
      return utils.retry(function() {
        ...
        .then(function(spans) {
          ...
          expect(spans).to.have.lengthOf(4, 'Spans: ' +
            JSON.stringify(spans, 0, 2));
        });
      });
    });
